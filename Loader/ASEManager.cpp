...
namespace DataLoader
{
	...
	void ASEManager::ParseGeomObject()
	{
		...
		while (true)
		{
			...
			{
				geoObject.tmAnimation = ParseTMAnimation();
	...
	ASEParsingData::TMAnimation ASEManager::ParseTMAnimation()
	{
		...
		while (true)
		{
			...
			if (reservedWord == TOKENR_CONTROL_POS_TRACK)
	...
	void ASEManager::ParseTMAnimationControlRotTrack(ASEParsingData::TMAnimation& parsingAnim)
	{
		...
		while (true)
		{
			...
			else if (reservedWord == TOKENR_CONTROL_ROT_SAMPLE)
			{
				...
				Quaternion quat = XMQuaternionRotationAxis(XMLoadFloat3(&tickAxis), tickAngle);

				// 각도의 애니메이션은 누적 방식으로 저장된다.
				if (!parsingAnim.controlRotSamples.empty())
				{
					const auto& quatPrevTick = parsingAnim.controlRotSamples.back().second;

					quat = quatPrevTick * quat;
				}
	...
	ASEParsingData::NodeTM ASEManager::ParseNodeTM()
	{
		...
		while (true)
		{
			...
			else if (reservedWord == TOKENR_TM_ROW3)
			{
				...
				const float det = XMMatrixDeterminant(XMLoadFloat4x4(&ret.tm)).m128_f32[0];
				...
				{
					XMMATRIX _Temp = XMLoadFloat4x4(&ret.tm);
					XMVECTOR _Scale, _Rot, _Tras;
					XMMatrixDecompose(&_Scale, &_Rot, &_Tras, _Temp);

					_Scale = -1 * _Scale;

					XMStoreFloat4x4(&ret.tm,
						XMMatrixScaling(_Scale.m128_f32[0], _Scale.m128_f32[1], _Scale.m128_f32[2])
						* XMMatrixRotationQuaternion(_Rot)
						* XMMatrixTranslation(_Tras.m128_f32[0], _Tras.m128_f32[1], _Tras.m128_f32[2]));
				}
			}
		}
	...
	void ASEManager::ParseHelperObject()
	{
		...
		while (true)
		{
			...
			if (reservedWord == TOKENR_NODE_NAME)
				...
				helperObject.tmAnimation = ParseTMAnimation();
		}
	...
	void ASEManager::ParseShapeObject()
	{
		...
		while (true)
		{
			...
			if (reservedWord == TOKENR_NODE_NAME)
				...
				shapeObject.tmAnimation = ParseTMAnimation();
		}
